<?php
session_start();
require_once __DIR__ . '/../../includes/Database.php';
require_once __DIR__ . '/../../includes/Auth.php';

$auth = new Auth();
$db = Database::getInstance();

requireAuth();

$pageTitle = 'Infrastructure Devices - InfraData Manager';
$message = '';
$messageType = '';

// Handle form submissions
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'create':
                $data = [
                    'device_name' => $_POST['device_name'] ?? '',
                    'machine_type' => $_POST['machine_type'] ?? '',
                    'brand' => $_POST['brand'] ?? '',
                    'ip_address' => $_POST['ip_address'] ?? '',
                    'purchase_year' => $_POST['purchase_year'] ?? '',
                    'vendor' => $_POST['vendor'] ?? '',
                    'partner_name' => $_POST['partner_name'] ?? '',
                ];
                
                // Add custom fields
                $customFields = $db->read('custom_fields');
                foreach ($customFields as $field) {
                    if (isset($_POST['cf_' . $field['id']])) {
                        $data['cf_' . $field['id']] = $_POST['cf_' . $field['id']];
                    }
                }
                
                $db->insert('devices', $data);
                $message = 'Device added successfully!';
                $messageType = 'success';
                break;
                
            case 'update':
                $id = $_POST['id'] ?? '';
                $data = [
                    'device_name' => $_POST['device_name'] ?? '',
                    'machine_type' => $_POST['machine_type'] ?? '',
                    'brand' => $_POST['brand'] ?? '',
                    'ip_address' => $_POST['ip_address'] ?? '',
                    'purchase_year' => $_POST['purchase_year'] ?? '',
                    'vendor' => $_POST['vendor'] ?? '',
                    'partner_name' => $_POST['partner_name'] ?? '',
                ];
                
                // Add custom fields
                $customFields = $db->read('custom_fields');
                foreach ($customFields as $field) {
                    if (isset($_POST['cf_' . $field['id']])) {
                        $data['cf_' . $field['id']] = $_POST['cf_' . $field['id']];
                    }
                }
                
                $db->update('devices', $id, $data);
                $message = 'Device updated successfully!';
                $messageType = 'success';
                break;
                
            case 'delete':
                $id = $_POST['id'] ?? '';
                $db->delete('devices', $id);
                $message = 'Device deleted successfully!';
                $messageType = 'success';
                break;
        }
    }
}

$devices = $db->read('devices');
$customFields = $db->read('custom_fields');

ob_start();
?>

<div class="space-y-6">
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white">
                <i class="fa-duotone fa-server mr-3 text-primary"></i>Infrastructure Devices
            </h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Manage physical devices and machines</p>
        </div>
        <?php if ($auth->isAdmin() || $auth->isOperator()): ?>
        <button onclick="openModal('create')" 
                class="px-6 py-3 bg-primary hover:bg-blue-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5">
            <i class="fa-duotone fa-plus mr-2"></i>Add Device
        </button>
        <?php endif; ?>
    </div>

    <?php if ($message): ?>
    <div class="p-4 bg-<?= $messageType === 'success' ? 'green' : 'red' ?>-100 dark:bg-<?= $messageType === 'success' ? 'green' : 'red' ?>-900 border border-<?= $messageType === 'success' ? 'green' : 'red' ?>-400 dark:border-<?= $messageType === 'success' ? 'green' : 'red' ?>-700 text-<?= $messageType === 'success' ? 'green' : 'red' ?>-700 dark:text-<?= $messageType === 'success' ? 'green' : 'red' ?>-200 rounded-lg flex items-center">
        <i class="fa-duotone fa-<?= $messageType === 'success' ? 'check-circle' : 'triangle-exclamation' ?> mr-2"></i>
        <?= htmlspecialchars($message) ?>
    </div>
    <?php endif; ?>

    <!-- Devices Grid -->
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        <?php if (empty($devices)): ?>
        <div class="col-span-full glass-card rounded-2xl p-12 text-center">
            <i class="fa-duotone fa-server text-6xl text-gray-300 dark:text-gray-600 mb-4"></i>
            <p class="text-gray-500 dark:text-gray-400 text-lg">No devices registered yet.</p>
            <?php if ($auth->isAdmin() || $auth->isOperator()): ?>
            <button onclick="openModal('create')" class="mt-4 px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-700">
                <i class="fa-duotone fa-plus mr-2"></i>Add First Device
            </button>
            <?php endif; ?>
        </div>
        <?php else: ?>
            <?php foreach ($devices as $device): ?>
            <div class="glass-card rounded-2xl p-6 hover:shadow-xl transition-all">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white"><?= htmlspecialchars($device['device_name']) ?></h3>
                        <p class="text-sm text-primary font-semibold"><?= htmlspecialchars($device['machine_type']) ?></p>
                    </div>
                    <div class="flex gap-2">
                        <?php if ($auth->isAdmin() || $auth->isOperator()): ?>
                        <button onclick='openModal("edit", <?= json_encode($device) ?>)' 
                                class="p-2 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900 rounded-lg transition-colors">
                            <i class="fa-duotone fa-pen-to-square"></i>
                        </button>
                        <?php endif; ?>
                        <?php if ($auth->isAdmin()): ?>
                        <form method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                            <input type="hidden" name="action" value="delete">
                            <input type="hidden" name="id" value="<?= $device['id'] ?>">
                            <button type="submit" class="p-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-900 rounded-lg transition-colors">
                                <i class="fa-duotone fa-trash"></i>
                            </button>
                        </form>
                        <?php endif; ?>
                    </div>
                </div>
                
                <div class="space-y-2 text-sm">
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-tag w-6 text-accent"></i>
                        <span>Brand: <?= htmlspecialchars($device['brand']) ?></span>
                    </div>
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-network-wired w-6 text-primary"></i>
                        <span>IP: <?= htmlspecialchars($device['ip_address']) ?></span>
                    </div>
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-calendar w-6 text-green-600"></i>
                        <span>Purchased: <?= htmlspecialchars($device['purchase_year']) ?></span>
                    </div>
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-store w-6 text-purple-600"></i>
                        <span>Vendor: <?= htmlspecialchars($device['vendor']) ?></span>
                    </div>
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-handshake w-6 text-orange-600"></i>
                        <span>Partner: <?= htmlspecialchars($device['partner_name']) ?></span>
                    </div>
                    
                    <?php foreach ($customFields as $field): ?>
                    <?php if (isset($device['cf_' . $field['id']]) && !empty($device['cf_' . $field['id']])): ?>
                    <div class="flex items-center text-gray-600 dark:text-gray-400">
                        <i class="fa-duotone fa-field w-6 text-gray-400"></i>
                        <span><?= htmlspecialchars($field['name']) ?>: <?= htmlspecialchars($device['cf_' . $field['id']]) ?></span>
                    </div>
                    <?php endif; ?>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
</div>

<!-- Modal -->
<div id="deviceModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="glass-card rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-inherit">
            <h2 id="modalTitle" class="text-2xl font-bold text-gray-800 dark:text-white">Add Device</h2>
            <button onclick="closeModal()" class="text-gray-600 dark:text-gray-300 hover:text-red-600">
                <i class="fa-duotone fa-times text-2xl"></i>
            </button>
        </div>
        
        <form method="POST" class="p-6 space-y-4">
            <input type="hidden" name="action" id="formAction" value="create">
            <input type="hidden" name="id" id="deviceId">
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Device Name *</label>
                    <input type="text" name="device_name" id="deviceName" required 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Machine Type *</label>
                    <input type="text" name="machine_type" id="machineType" required 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent"
                           placeholder="e.g., Server, Router, Switch">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Brand *</label>
                    <input type="text" name="brand" id="brand" required 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">IP Address *</label>
                    <input type="text" name="ip_address" id="ipAddress" required 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Purchase Year</label>
                    <input type="text" name="purchase_year" id="purchaseYear" 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Vendor</label>
                    <input type="text" name="vendor" id="vendor" 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Partner Name</label>
                    <input type="text" name="partner_name" id="partnerName" 
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
            </div>
            
            <!-- Custom Fields -->
            <?php if (!empty($customFields)): ?>
            <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                <h4 class="font-semibold text-gray-700 dark:text-gray-300 mb-3">Custom Fields</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php foreach ($customFields as $field): ?>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            <?= htmlspecialchars($field['name']) ?>
                        </label>
                        <input type="text" name="cf_<?= $field['id'] ?>" 
                               class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php endif; ?>
            
            <div class="flex gap-3 pt-4">
                <button type="submit" 
                        class="flex-1 py-3 px-4 bg-primary hover:bg-blue-700 text-white font-semibold rounded-lg shadow-lg transition-all">
                    <i class="fa-duotone fa-save mr-2"></i>Save
                </button>
                <button type="button" onclick="closeModal()" 
                        class="px-6 py-3 bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600 text-gray-800 dark:text-white font-semibold rounded-lg transition-all">
                    Cancel
                </button>
            </div>
        </form>
    </div>
</div>

<script>
function openModal(mode, data = null) {
    const modal = document.getElementById('deviceModal');
    const title = document.getElementById('modalTitle');
    const action = document.getElementById('formAction');
    const deviceId = document.getElementById('deviceId');
    
    if (mode === 'edit' && data) {
        title.textContent = 'Edit Device';
        action.value = 'update';
        deviceId.value = data.id;
        
        document.getElementById('deviceName').value = data.device_name || '';
        document.getElementById('machineType').value = data.machine_type || '';
        document.getElementById('brand').value = data.brand || '';
        document.getElementById('ipAddress').value = data.ip_address || '';
        document.getElementById('purchaseYear').value = data.purchase_year || '';
        document.getElementById('vendor').value = data.vendor || '';
        document.getElementById('partnerName').value = data.partner_name || '';
    } else {
        title.textContent = 'Add Device';
        action.value = 'create';
        deviceId.value = '';
        
        document.getElementById('deviceName').value = '';
        document.getElementById('machineType').value = '';
        document.getElementById('brand').value = '';
        document.getElementById('ipAddress').value = '';
        document.getElementById('purchaseYear').value = '';
        document.getElementById('vendor').value = '';
        document.getElementById('partnerName').value = '';
    }
    
    modal.classList.remove('hidden');
}

function closeModal() {
    document.getElementById('deviceModal').classList.add('hidden');
}

// Close modal when clicking outside
document.getElementById('deviceModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closeModal();
    }
});
</script>

<?php
$content = ob_get_clean();
include __DIR__ . '/../../templates/layout.php';
?>
