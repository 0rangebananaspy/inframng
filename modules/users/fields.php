...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    switch ($_POST['action']) {
        ...
        case 'update':
            ...
            $db->update('custom_fields', $id, $data);
...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white">
                <i class="fa-duotone fa-list-check mr-3 text-pink-600"></i>Custom Fields
            </h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Define custom fields for device data entry</p>
        </div>
        <button onclick="openModal('create')"
                class="px-6 py-3 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="col-span-full glass-card rounded-2xl p-12 text-center">
            ...
            <button onclick="openModal('create')" class="mt-4 px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700">
                <i class="fa-duotone fa-plus mr-2"></i>Create First Field
            </button>
        </div>
        <?php else: ?>
            ...
            <div class="glass-card rounded-2xl p-6 hover:shadow-xl transition-all">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white"><?= htmlspecialchars($field['name']) ?></h3>
                        <span class="inline-block px-2 py-1 bg-pink-100 dark:bg-pink-900 text-pink-700 dark:text-pink-300 text-xs rounded-full mt-1">
                            <?= ucfirst(htmlspecialchars($field['type'])) ?>
                        </span>
                    </div>
                    <div class="flex gap-2">
                        <button onclick='openModal("edit", <?= json_encode($field) ?>)'
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400"><?= htmlspecialchars($field['description']) ?></p>
                <?php endif; ?>

                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
                    Created: <?= date('M d, Y', strtotime($field['created_at'])) ?>
                </div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
...
<div id="fieldModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="glass-card rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-inherit">
            <h2 id="modalTitle" class="text-2xl font-bold text-gray-800 dark:text-white">Add Custom Field</h2>
        ...
        <form method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="fieldName" required
                       class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-pink-600 focus:border-transparent"
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit"
                        class="flex-1 py-3 px-4 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-lg shadow-lg transition-all">
...
function openModal(mode, data = null) {
    ...
    if (mode === 'edit' && data) {
        ...
        document.getElementById('fieldType').value = data.type || 'text';
